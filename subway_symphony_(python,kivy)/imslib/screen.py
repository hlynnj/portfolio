...
class Screen(Widget):
    ...
    def switch_to(self, screen_name, kwargs=None):
...
class ScreenManager(BaseWidget):
    ...
    def switch_to(self, screen_name, kwargs=None):
        ...
        :param screen_name: the name of the screen
        """
        kivyClock.schedule_once(lambda dt: self._switch_to(screen_name, kwargs))
    ...
    def _switch_to(self, screen_name, kwargs=None):
        if self.cur_screen:
        ...
        if kwargs:
            if 'score' in kwargs:
                self.cur_screen.set_score(kwargs['score'])